...
if ( ! class_exists( 'YITH_WCStripe_Gateway' ) ) {
	...
	class YITH_WCStripe_Gateway extends WC_Payment_Gateway_CC {
		...
		protected function pay( $order = null ) {
			...
			$params = array(
				'amount'      => YITH_WCStripe::get_amount( $order->get_total(), $currency, $order ), // Amount in cents!
			...
			if ( isset( $_REQUEST['reference'] ) && isset( $_REQUEST['signature'] ) && isset( $_REQUEST['stripeToken'] ) ) {
				...
				if ( $signature === $_REQUEST['signature'] ) {
					...
					try {
						...
						if ( $body ) {
							$err = $body['error'];
							if ( isset( $err['code'] ) && isset( $this->errors[ $err['code'] ] ) && apply_filters( 'yith_wcstripe_use_plugin_error_codes', true ) ) {
								$message = apply_filters( 'yith_wcstripe_error_message', $this->errors[ $err['code'] ], $message, $err['code'] );
							}
						}
